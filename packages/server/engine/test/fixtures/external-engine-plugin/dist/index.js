...
function enginePlugin({ config, logger }) {
  const parsedConfig = parseConfig({ config })
  const rules = parsedConfig.rules.map((rule) => ({
    name: rule.name,
  ...
  logger.info({
    ...
    ruleNames,
    pieceNameCount: parsedConfig.pieceNames === undefined ? 0 : parsedConfig.pieceNames.length,
  }, 'Loaded redact input strings engine plugin fixture')

  return {
    name: 'external-redact-input-strings-fixture',
    version: '0.1.0',
...
function parseOptionalStringArray({ value, propertyName }) {
  if (value === undefined) {
    return undefined
  }
...
function parseOptionalRules({ value }) {
  if (value === undefined) {
    return []
  }
...
function parseRequiredString({ value, propertyName, index }) {
...
function parseOptionalString({ value, propertyName, index }) {
  if (value === undefined) {
    return undefined
  }
...
function normalizeFlags({ flags }) {
  const configuredFlags = flags === undefined ? '' : flags
...
function buildMiddleware({ rules, pieceNames }) {
  return {
    name: 'external-redact-input-strings-middleware',
    ...(pieceNames === undefined ? {} : { match: { pieceName: pieceNames } }),
    before: ({ input, canReplaceInput }) => {
      if (!canReplaceInput || rules.length === 0 || !hasPropsValue(input)) {
        return undefined
      }

      const result = redactValue({
        value: input.propsValue,
        rules,
        depth: 0,
        visitedCount: 0,
      })
...
function redactValue({ value, rules, depth, visitedCount }) {
...
function redactString({ value, rules, visitedCount }) {
...
function redactArray({ value, rules, depth, visitedCount }) {
  const nextArray = []
...
function redactPlainObject({ value, rules, depth, visitedCount }) {
  const nextObject = {}
  let nextVisitedCount = visitedCount
  let redactionCount = 0
  let changed = false
...
function hasPropsValue(value) {
  return isRecord(value) && Object.prototype.hasOwnProperty.call(value, 'propsValue')
}
